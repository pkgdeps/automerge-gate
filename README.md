...
The gate lists the workflow runs for the SHA (`GET /repos/{owner}/{repo}/actions/runs?head_sha=...`) and treats a workflow run as replaced when a newer run with the same workflow file and the same event exists. In a replaced run, the gate ignores a `cancelled` check_run, and ignores any other check_run when the newest run has a job with the same name that finished with a result other than `skipped`. No configuration is needed.

- A job whose result depends on the event payload is judged by the newest run. For example, a job that fails until a PR label is added fails in the first `pull_request` run, and passes in the run started by `labeled`; the gate uses the passing result. Re-running the first run does not help here, because a re-run replays the original payload.
- A job that failed in the older run still fails the gate when the newest run skips that job (for example via `if:`), because `skipped` does not show that the failure is fixed.
